...
openModalButtons.forEach(button => {
	button.addEventListener('click', (e) => {
        ...
        if(popup.id == 'modal_edit_player'){
			let player = JSON.stringify({_id: savedId})
			let request = new XMLHttpRequest();
			var playerInfo = null;
            console.log(savedId)
			request.open("GET", "/api/players/" + savedId, true);   
			request.setRequestHeader("Content-Type", "application/json");
			request.addEventListener("load", function () {
              
				playerInfo = JSON.parse(request.response);
                console.log(playerInfo)
				document.getElementById('player_edit_name').value = playerInfo.name
				document.getElementById('player_edit_surname').value = playerInfo.surname
				document.getElementById('player_edit_phone').value = playerInfo.phone
			});
			request.send(player);
			popup.querySelector("#edit_player_form").setAttribute("action", "/players/update/" + savedId)
			
		}
		if(popup.id == 'delete_team'){
			popup.querySelector("#delete_team_form").setAttribute("action", "/players/delete/" + savedId)
		}
		openModal(popup)
	})
})
